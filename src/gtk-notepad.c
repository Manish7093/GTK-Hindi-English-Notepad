...
void gtk_notepad_set_title(const char* filename) {
    ...
    char *_title = malloc(strlen(gettext(" - GTK Notepad")) + strlen(fn) + 1);
    strcpy(_title, fn);
    if (slash)
        for (i=0; i <= index; i++, fn--);

    strcat(_title, gettext(" - GTK Notepad"));
...
void gtk_notepad_show_about_box(void) {
    ...
    gtk_about_dialog_set_name(dlg, gettext("GTK Notepad"));
    gtk_about_dialog_set_version(dlg, gettext("0.1"));
    gtk_about_dialog_set_comments(dlg, gettext("GTK Notepad is a text editing "
                            "program written in C and GTK."));
    gtk_about_dialog_set_website(dlg,
        "https://github.com/Manish7093");
...
int gtk_notepad_ask_save_cancel(void) {
    ...
    dialog = gtk_message_dialog_new(gwindow,
                ...
                GTK_BUTTONS_YES_NO,
                gettext("File has been modified. Do you want to save it?"));
    gtk_dialog_add_button(GTK_DIALOG(dialog),
                          gettext("Cancel"), GTK_RESPONSE_CANCEL);
    gtk_window_set_title(GTK_WINDOW(dialog), gettext("File has been modified"));
...
void gtk_notepad_select_font(void) {
    ...
    GtkWidget *dialog = gtk_font_selection_dialog_new(gettext("Select a font"));
...
void gtk_notepad_new(void) {
    ...
    gtk_window_set_title(gwindow, gettext("Untitled - GTK Notepad"));
}
...
void gtk_notepad_open(void) {
    ...
    dialog = gtk_file_chooser_dialog_new(gettext(gettext("Open a file")),
                                         gwindow,
                                         action,
                                         "_Open",
                                         GTK_RESPONSE_ACCEPT,
...
void gtk_notepad_saveas(void) {
    ...
    dialog = gtk_file_chooser_dialog_new(gettext("Save the file"),
                                         gwindow,
                                         action,
                                         "_Save",
                                         GTK_RESPONSE_ACCEPT,
...
void setup_menubar(void) {
    ...
    file = gtk_menu_item_new_with_mnemonic(gettext("_File"));
    new  = gtk_menu_item_new_with_mnemonic(gettext("_New"));
    open = gtk_menu_item_new_with_mnemonic(gettext("_Open"));
    save = gtk_menu_item_new_with_mnemonic(gettext("_Save"));
    saveas = gtk_menu_item_new_with_mnemonic(gettext("Save _as..."));
    quit = gtk_menu_item_new_with_mnemonic(gettext("_Quit"));

    editmenu = gtk_menu_new();
    edit = gtk_menu_item_new_with_mnemonic(gettext("_Edit"));
    cut  = gtk_menu_item_new_with_mnemonic(gettext("Cu_t"));
    copy = gtk_menu_item_new_with_mnemonic(gettext("_Copy"));
    paste = gtk_menu_item_new_with_mnemonic(gettext("_Paste"));
    delete = gtk_menu_item_new_with_mnemonic(gettext("_Delete"));
    selectall = gtk_menu_item_new_with_mnemonic(gettext("_Select All"));
    time_date = gtk_menu_item_new_with_mnemonic(gettext("_Time/Date"));
    wll  = gtk_check_menu_item_new_with_mnemonic(gettext("_Wrap long "
                                                 "lines"));
    select_font = gtk_menu_item_new_with_mnemonic(gettext("_Font..."));

    helpmenu = gtk_menu_new();
    help = gtk_menu_item_new_with_mnemonic(gettext("_Help"));
    about = gtk_menu_item_new_with_mnemonic(gettext("_About"));
...
int main(int argc, char* argv[]) {
    ...
    setlocale(LC_ALL, "" );
    textdomain("notepadi18");

    // Place to look for MO file
    bindtextdomain("notepadi18", "./locale/");  
